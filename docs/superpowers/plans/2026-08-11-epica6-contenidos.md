...
- **Tarea 4 — `remark-gfm` no estaba en el plan.** El texto original de esta tarea (y de la spec, §3) solo mencionaba `react-markdown` como dependencia nueva. Al implementar el detalle se agregó también `remark-gfm` como `remarkPlugins` de `ReactMarkdown`, porque CommonMark puro no soporta tablas y tanto el CSS de `markdown-body` como el test 1 de la Tarea 6 (una tabla en el cuerpo debe verse como `<table>`, no como texto con `|`) ya asumían que las tablas se renderizan. Sin el plugin, ese test falla. La spec (§3) se actualizó para reflejar esta dependencia real.
- **Tarea 6, test 2 — la versión literal del plan tenía una carrera con el redirect de `/e2e-login`.** `/e2e-login` inicia sesión del lado del cliente (`signInWithPassword`) y solo después redirige vía `window.location.href`, lo cual ocurre DESPUÉS de que `page.goto()` ya se resolvió. La versión original del test hacía, tras el login, una aserción NEGATIVA (que un control no exista) seguida de un segundo `page.goto()` — la aserción negativa es trivialmente cierta incluso en la página de login a medio cargar, así que no esperaba nada real, y el segundo `page.goto()` a veces llegaba al servidor antes de que la cookie de sesión existiera, aterrizando en `/login` en vez de la página esperada. Se reprodujo en 9 de 9 corridas. El test se reescribió para que la primera aserción tras el login sea POSITIVA (algo que solo es cierto una vez completada la redirección), lo cual fuerza una espera real antes de cualquier navegación subsiguiente. El mismo patrón se siguió en los tests de guard de ruta agregados después (`/nuevo` y `/[id]/editar`).
