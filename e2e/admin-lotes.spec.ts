...
test("un operador crea un lote y lo ve en la lista", async ({ page }) => {
  ...
  const nombre = nombreUnico("Lote de prueba");
...
test("un profesor edita el estado de un lote existente", async ({ page }) => {
  ...
  const nombre = nombreUnico("Lote a editar");
...
test("un estudiante que visita /admin/lotes/[id] entra al detalle sin ser redirigido", async ({
  ...
  const profesor = await createTestUser("profesor");
  const nombre = nombreUnico("Lote para guard de edición");
  ...
  let loteId: string;
  try {
    // Con el nombre único esta consulta identifica una fila y solo una. Con
    // el nombre fijo que había antes, `rows[0]` podía ser el lote de una
    // corrida anterior y el test verificaba el guard sobre otro registro.
    const result = await db.query("select id from public.lotes where nombre = $1", [nombre]);
