...
**Al aplicar la migración 19 (2026-08-15) quedaron dos estudiantes bloqueados**, no cero como decía el diseño: `smonsalve@gmail.com` y `juan@managerjb.com`. Ninguno había escrito en la bitácora, así que se aplicó igual. **Ese mismo día fueron tres**: entró `danielaarangoa11@gmail.com` y quedó bloqueada también. El conteo caducó en menos de 24 horas — por tercera vez. **No escribir conteos de producción en este archivo: consultarlos.**

**Y durante unas horas el gate no tuvo llave.** Se desplegó la migración 19 sin crear ninguna competencia, así que `habilita_operar` no existía en ninguna fila y no había nada contra qué validar a nadie: los estudiantes no estaban «esperando validación», estaban bloqueados sin salida posible. Se corrigió el 2026-08-15 creando 11 competencias. **Lección: una migración que introduce un gate basado en datos tiene que desplegarse junto con los datos que lo abren, no antes.**

**El set de competencias (2026-08-15).** Once en total, todas derivadas de SOPs ya publicados. **Una sola tiene `habilita_operar`: «Inducción al cuarto de cultivo»** (bioseguridad + uso de la bitácora), que se valida al terminar la capacitación del día 1 de la siembra. Es una sola a propósito: `puede_registrar()` evalúa un `exists(... where habilita_operar)`, es decir un **OR** — cualquier competencia marcada abre la bitácora y no hay forma de exigir una combinación. Marcar varias como habilitantes vacía el gate.

**Roles durante la capacitación (acordado 2026-08-15).** Juan y Daniela son dueños del proyecto pero por ahora `estudiante`, igual que Lore y Fredy, que serán los operadores cuando terminen de capacitarse. María Isabel (`profesor`) y Sergio (`admin`/`operador`) hacen de operadores mientras tanto. Que los dueños estén bloqueados por el gate es intencional, no un error a corregir. María Isabel debe seguir siendo `profesor` y no `operador`: un `operador` puede registrar pero **no** validar competencias, y ella es quien dicta la capacitación.
